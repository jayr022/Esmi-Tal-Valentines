...
function initPuzzle() {
    tiles = [];
    const img = new Image();
    img.src = 'cartoon.png'; // For puzzle tiles (450x450px)
    img.onload = () => {
...
function handleEnvelopeClick(e) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            initPuzzle();
            // Add Spotify player dynamically for auto-play on envelope open
            const spotifyPlayer = document.createElement('iframe');
            spotifyPlayer.id = 'spotify-player';
            spotifyPlayer.src = 'https://open.spotify.com/embed/track/04Aw2jdHNkLAR9LWUIGf2b?autoplay=1';
            spotifyPlayer.width = '0';
            spotifyPlayer.height = '0';
            spotifyPlayer.frameborder = '0';
            spotifyPlayer.allowtransparency = 'true';
            spotifyPlayer.allow = 'autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture';
            spotifyPlayer.loading = 'lazy';
